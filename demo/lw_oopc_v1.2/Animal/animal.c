...
Fish* Fish_new() { 
    struct Fish *cthis; 
    cthis = (struct Fish*)malloc(sizeof(struct Fish)); 
    if(!cthis) { 
        return 0; 
    } 
    Fish_ctor(cthis); 
...
void Fish_ctor(Fish* cthis) {
//SUPER_CTOR(Animal);
Animal_ctor(((Animal*)(&(cthis->Animal))));
//FUNCTION_SETTING(Animal.eat, Fish_eat);
cthis->Animal.eat = Fish_eat;
//FUNCTION_SETTING(Animal.breathe, Fish_breathe);
cthis->Animal.breathe = Fish_breathe;
//FUNCTION_SETTING(IMoveable.move, Fish_move);
cthis->IMoveable.move = Fish_move;
//FUNCTION_SETTING(init, Fish_init);
cthis->init = Fish_init;
//END_CTOR
}
