...
void Expr_node_ctor(Expr_node* t); 
int Expr_node_dtor(Expr_node* t); 
void Expr_node_delete(Expr_node* t); 
struct Expr_node
{
	int use;        // 引用计数
...
Expr* Expr_new(lw_oopc_file_line_params); 
void Expr_ctor(Expr* t); int Expr_dtor(Expr* t); 
void Expr_delete(Expr* t); 
struct Expr
{
	int use;        // 引用计数
	Expr_node* p;   // 子树的根节点
